...
export default function streamSocket(io) {
  io.on("connection", (socket) => {
    console.log("New stream socket connection established:", socket.id);
    ...
    // Fallback Stream Relay: Video Frame (JPEG Data URL)
    socket.on("stream_frame", ({ streamId, frame }) => {
      socket.to(`stream_${streamId}`).emit("stream_frame", {
        senderSocketId: socket.id,
        frame,
      });
    });

    // Fallback Stream Relay: Audio Chunk (Base64 WAV)
    socket.on("stream_audio", ({ streamId, audio }) => {
      socket.to(`stream_${streamId}`).emit("stream_audio", {
        senderSocketId: socket.id,
        audio,
      });
    });
